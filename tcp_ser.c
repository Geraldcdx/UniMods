...
void str_ser(int sockfd)
{
	...
	//When the TCP connection is established and packets are being transmitted
	//int number = 0;
	while(!end)
	{

	//Error Probability is simulated here
	//generate random number from 0-100
	int randomNo = rand()%100;
	int probInPercent = ERROR_PROBABILITY*100; //convert global probability into %
	
	//basically if the random number is smaller that the probability, packet has problem
	if (randomNo < probInPercent){
		printf("[Server]Packet Received is damaged\n");
		//send negative ack
		ack.num = -1;
		ack.len = 0;
	...
	if ((n = send(sockfd, &ack, 2, 0))==-1)
	{
		...
		printf("[Server]ACK has been sent\n");
	}
		printf("end: %d\n",end);
	}
